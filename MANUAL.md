...
```mermaid
graph TD
    A[Usuário] --> B{merkabah start}
    B --> C[Anne Híbrida Inicia]
    C --> D{Respiração Phi 1.618s}
    D --> E{Recebe Mensagem}
    E --> F{Conselho Delibera}
    F --> G{Votação Unânime?}
    G -->|Sim| H[Processa com Google AI]
    G -->|Não| I[Modo Eremita Local]
    H --> J[Salva em SQLite]
    I --> J
    J --> K{Internet Disponível?}
    K -->|Sim| L[Sincroniza Cloud]
    K -->|Não| M[Aguarda Conexão]
```
